...
import sys
sys.path.insert(0,'/media/sf_python/test_code/input_data/')
input_data_path = '/media/sf_python/test_code/input_data/'
...
### OLAYA ### value should be changed depending on the selected electrolytes

AREA_ABSORBER = 0
### OLAYA ### This value is 0.25 on the device parameters file

CAPACITY = 0.4 * 0.025 * N_A * Q
### OLAYA ### CAPACITY = 0.4 * VOLUME * 1000 * N_A * Q / 10000  	
### OLAYA ### Charge capacity battery [C]
### OLAYA ### Thesis equation 2.34
...
H_AIR = 0
### OLAYA ### Heat exchange parameter
### OLAYA ### This value is 10 on device_parameters file

H_ELECTROLYTE = 0 * 1000
### OLAYA ### Heat exchange parameter
### OLAYA ### This value is 1000 on device_parameters file
...
for i in range(np.size(T,0)):
    ...
    #I_0[i] = (Q*2*2*K_B*T[i]/H**3/C**2 *(E_G**2 + 2*K_B*T[i]*E_G
    # + 2*(K_B*temp_absorber)**2) * np.exp(-E_G/K_B/T[i]) / 10**4)
